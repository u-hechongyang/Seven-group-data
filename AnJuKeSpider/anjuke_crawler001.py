...
#解析各个街道的内容
def detail_street_parser(street_url_lists):
    global city
    if not street_url_lists:
        print("detail url 为空")
        return None
    headers =  {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36',
    ...
    for detail_url in street_url_lists:
        try:
            t = uniform(1, 3)
            time.sleep(t)
    ...
    for city in city_list:
        ...
        for index in range(2012, 2019):
            ...
            for municipal_url in municipal_list:
                street_url_list = get_street_page_url(municipal_url)
                detail_street_parser(street_url_list)
...
if __name__ == '__main__':
    with open('data/anhui001.csv','w',encoding="utf-8") as f:
